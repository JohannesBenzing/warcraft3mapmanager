...
StarRatingDelegate::StarRatingDelegate(QObject *parent)
    : QItemDelegate(parent), m_star(QPixmap(":img/star.png"))
{
}
...
void StarRatingDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    int height = m_star.height();
    int x = option.rect.x();
    int y = option.rect.y() + (option.rect.height() / 2) - (height / 2);
    for (int i = 0; i < rating; ++i) {
        painter->drawPixmap(x, y, m_star);
        x += width;
    }
...
{
    return QSize(5 * m_star.width(), m_star.height()) + QSize(1, 1);
}
